...
export const GameDetails = () => {
  ...
  return (
    <div className="row justify-content-center gamedet m-3 ">
      ...
      <div>
        <ul>
          {store.currentItem.comments.map((comment, index) => (
          <li key={index} >
            <img src={comment.user_pfp} className="card-img-top" alt={comment.username} style={{ width: "2%", height: "auto", borderRadius: "90px"}}/>
            {comment.username}{comment.body}</li>))}
        </ul>
      </div>
    </div>
  );
